...
void save(char *fileName) {
    ...
    for (i=0; i<n; i++) {
        fprintf(fp, "%s#", (directory[i]->name != NULL ? directory[i]->name : (char *)" "));
        fprintf(fp, "%s#", (directory[i]->number != NULL ? directory[i]->number : (char *)" "));
        fprintf(fp, "%s#", (directory[i]->email != NULL ? directory[i]->email : (char *)" "));
        fprintf(fp, "%s#\n", (directory[i]->group != NULL ? directory[i]->group : (char *)" "));
    }
    fclose(fp);
}
...
void handle_add(char * name)  {
    char buffer[BUFFER_LENGTH];
    char *number = NULL, *email = NULL, *group = NULL;

    printf("  Phone: ");
    read_line(stdin, buffer, BUFFER_LENGTH);
    if (strlen(buffer)>0)
        number = strdup(buffer);
    printf("  Email: ");
    read_line(stdin, buffer, BUFFER_LENGTH);
    if (strlen(buffer)>0)
        email = strdup(buffer);
